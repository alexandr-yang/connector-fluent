...
Функция АдресРесурса(АдресРесурса) Экспорт
	...
	Контекст.АдресРесурса = АдресРесурса;
	Возврат ЭтотОбъект;
...
Функция Параметры(Параметры) Экспорт
	...
	Если КонструкторHTTPСлужебный.ЕстьЭлементКоллекции(Параметры, "ЗащищенноеСоединение") Тогда
...
//  Строка
//
Функция ПодготовитьПорт(Порт)
	
	Возврат XMLСтрока(Порт);
...
//  Строка
//
Функция ПодготовитьСервер(Сервер)
	
	НовыйСервер = СокрЛП(НРег(Сервер));
	НовыйСервер = СтрЗаменить(НовыйСервер, "https://", "");
	НовыйСервер = СтрЗаменить(НовыйСервер, "http://", "");
	
	Если СтрЗаканчиваетсяНа(НовыйСервер, "/") Тогда
		НовыйСервер = Лев(НовыйСервер, СтрДлина(НовыйСервер) - 1);
	КонецЕсли;
...
Функция СформироватьУРЛ()
	...
	Фрагменты.Добавить(ПодготовитьСервер(Контекст.Сервер));
	
	Если ЗначениеЗаполнено(Контекст.Порт) Тогда
		Фрагменты.Добавить(":");
		Фрагменты.Добавить(ПодготовитьПорт(Контекст.Порт));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Контекст.АдресРесурса) Тогда
		Фрагменты.Добавить("/");
		Фрагменты.Добавить(ПодготовитьАдресРесурса(Контекст.АдресРесурса));
	КонецЕсли;
