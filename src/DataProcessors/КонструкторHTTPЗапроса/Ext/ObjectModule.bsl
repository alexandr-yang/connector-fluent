...
Функция ДобавитьФайл(Имя, ИмяФайла, Данные = Неопределено, Тип = Неопределено) Экспорт
	...
	ОтправляемыйФайл = КоннекторHTTP.НовыйОтправляемыйФайл(Имя, ИмяФайла,
		Данные, Тип); // BSLLS:MissingCommonModuleMethod-off
...
Функция Отправить() Экспорт
	...
	УРЛ = СформироватьУРЛ();
	Ответ = КоннекторHTTP.ВызватьМетод(_Контекст.Метод, УРЛ,
		_Контекст.ДополнительныеПараметры, _Контекст.Сессия); // BSLLS:MissingCommonModuleMethod-off
