...
Функция Параметры(Параметры) Экспорт
	...
	Если Параметры.Свойство("Метод") Тогда
	...
	Если Параметры.Свойство("АдресРесурса") Тогда
	...
	Если Параметры.Свойство("ЗащищенноеСоединение") Тогда
...
Функция Метод(Метод) Экспорт
	...
	Если HTTPМетоды.Найти(ВРег(Метод)) = Неопределено Тогда
		ОписаниеОшибки = СтрШаблон("Неизвестный HTTP-метод ""%1""", Метод);
...
Функция МетодGET() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодGET());
...
Функция МетодOPTIONS() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодOPTIONS());
...
Функция МетодHEAD() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодHEAD());
...
Функция МетодPOST() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодPOST());
...
Функция МетодPUT() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодPUT());
...
Функция МетодPATCH() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодPATCH());
...
Функция МетодDELETE() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодDELETE());
...
Функция МетодCONNECT() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодCONNECT());
...
Функция МетодTRACE() Экспорт
	...
	Возврат Метод(КонструкторHTTP.МетодTRACE());
...
Функция ТелоЗапроса(ТелоЗапроса, ЗаписатьJSON = Ложь) Экспорт
	...
	Если ЗаписатьJSON Тогда
		Контекст.ДополнительныеПараметры.JSON = ТелоЗапроса;
		Контекст.ДополнительныеПараметры.Данные = Неопределено;
	Иначе
		Контекст.ДополнительныеПараметры.JSON = Неопределено;
		Контекст.ДополнительныеПараметры.Данные = ТелоЗапроса;
	КонецЕсли;
...
Функция Таймаут(Таймаут) Экспорт
	...
	Контекст.ДополнительныеПараметры.Таймаут = Таймаут;
...
Функция КлиентскийСертификатSSL(Сертификат) Экспорт
	...
	Контекст.ДополнительныеПараметры.КлиентскийСертификатSSL = Сертификат;
...
Функция МаксимальноеВремяПовторов(Время) Экспорт
	...
	Контекст.ДополнительныеПараметры.МаксимальноеВремяПовторов = Время;
...
Функция МаксимальноеКоличествоПовторов(Количество) Экспорт
	...
	Контекст.ДополнительныеПараметры.МаксимальноеКоличествоПовторов = Количество;
...
Функция КоэффициентЭкспоненциальнойЗадержки(Коэффициент) Экспорт
	...
	Контекст.ДополнительныеПараметры.КоэффициентЭкспоненциальнойЗадержки = Коэффициент;
...
Функция Отправить() Экспорт
	...
	Возврат ОбработчикHTTPОтвета().Инициализировать(Ответ);
...
//  ОбработкаОбъект.КонструкторHTTPЗапроса
//
Функция Инициализировать(Сессия = Неопределено, Параметры = Неопределено) Экспорт
	
	Если ЭтоСтруктура(Сессия) Тогда
		Сессия(Сессия);
	КонецЕсли;
	
	Если ЭтоСтруктура(Параметры) Тогда
		Параметры(Параметры);
	КонецЕсли;
...
//  Массив из Строка
//
Функция ДоступныеHTTPМетоды() Экспорт
	
	ДоступныеHTTPМетоды = Новый Массив;
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодGET());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодOPTIONS());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодHEAD());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодPOST());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодPUT());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодPATCH());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодDELETE());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодCONNECT());
	ДоступныеHTTPМетоды.Добавить(КонструкторHTTP.МетодTRACE());
	
	Возврат ДоступныеHTTPМетоды;
...
//  ОбработкаОбъект.ОбработчикHTTPОтвета
//
Функция ОбработчикHTTPОтвета()
	
	Возврат КонструкторHTTP.ОбработчикHTTPОтвета();
...
HTTPМетоды = ДоступныеHTTPМетоды();
